...
Simulated separation: in production, `side="store:{store_id}"` rows live in
each store's Postgres, `side="cloud"` rows live in the cloud Aurora. In the
spike they coexist in a single SQLite instance so the whole flow can be
...
class SyncOutbox(models.Model):
    ...
    Retained after ACKED for pending-reconciliation visibility; purge is a
    separate operational concern, NOT on the correctness path.
    """
...
class PendingReconciliation(models.Model):
    ...
    large clock skews, etc. R4's territory but seeded here so R1 can prove
    "no silent data loss".
    """
