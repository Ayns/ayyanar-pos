"""
The IRP client state machine.

This is the centre of AYY-14. Every design choice here is driven by the
error taxonomy. The client is deliberately dumb: it does not know what a
GST invoice is. It knows how to call ``target.submit``, bucket the response
into a class, and decide what the submission does next.

State transitions (for one ``InvoiceSubmission``):

    pending       ──submit──▶ in_flight
    in_flight     ──ok/dup──▶ registered         (terminal)
    in_flight     ──TRANSIENT, budget left──▶ pending   (after backoff)
    in_flight     ──TRANSIENT, exhausted──▶ dead_lettered (retry_exhausted)
    in_flight     ──THROTTLE──▶ pending          (after Retry-After; budget NOT spent)
    in_flight     ──OUTAGE──▶ circuit_open       (tenant-wide cooldown)
    circuit_open  ──cooldown_elapsed──▶ pending
    in_flight     ──SECURITY──▶ re-auth, retry once; else dead_lettered
    in_flight     ──BUSINESS / SCHEMA──▶ dead_lettered  (terminal)

Retry budget: ``MAX_ATTEMPTS`` total HTTP attempts across TRANSIENT + SECURITY.
THROTTLE and OUTAGE do *not* consume budget — they are server-directed waits.
"""
...
# Retry budget + backoff — tunable, but these are the v1 defaults the plan
# document commits to.
MAX_ATTEMPTS = 6
BASE_BACKOFF_S = 2.0
MAX_BACKOFF_S = 300.0
...
class Clock:
    """
    Tests want deterministic time. Wall clock and monotonic are separate:
    - wall_now() — stored on rows (``started_at``, ``next_eligible_at``).
    - mono_now() — used for latency measurement.
    """
...
class IrpClient:
    """
    Owns one (target, auth provider) pair. Can be called concurrently — all
    mutation is either DB-level or localised to per-tenant ``TenantCircuit``
    objects, which use a dict lookup rather than locks because Django's DB
    layer is the actual synchronisation point.
    """
    ...
    def submit_pending(self, submission_id: int) -> str:
        """
        Drive a single submission one step. Returns the post-step status.
        Idempotent: safe to call on any status — terminal rows return their
        ...
        if circuit.is_open(wall_now):
            # Don't even attempt — the target will just burn latency.
    ...
    # ---- one HTTP attempt + outcome dispatch ----
    ...
    def _finish_ok(
        ...
        if ack_dt and not sub.ack_dt:
            # Best-effort parse; sandbox returns `YYYY-MM-DD HH:MM:SS` in IST.
            try:
    ...
    def _schedule_throttle(
        ...
        # THROTTLE doesn't consume retry budget — un-count this attempt.
    ...
    def _open_circuit(self, sub: InvoiceSubmission, now_wall: datetime) -> str:
        ...
        # OUTAGE also doesn't consume budget — it's a platform issue, not ours.
        sub.attempts = max(0, sub.attempts - 1)
        sub.status = SubmissionStatus.CIRCUIT_OPEN
        sub.next_eligible_at = circuit.open_until
        sub.save(update_fields=["status", "attempts", "next_eligible_at", "updated_at"])
