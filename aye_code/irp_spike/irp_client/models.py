...
1. ``InvoiceSubmission`` — the logical "we want this invoice registered". One
   row per source invoice per tenant, idempotent on (tenant_id, invoice_ref).
2. ``IrpAttempt`` — append-only record of every HTTP attempt against IRP.
   All latency / error-class stats come off this table.
3. ``DeadLetter`` — terminal-state record for submissions that must not be
   auto-retried. This is the manual-resubmit operator view.

The field names intentionally mirror the vocabulary the ops runbook uses.
"""
...
class InvoiceSubmission(models.Model):
    """
    One row per (tenant, invoice_ref). ``invoice_ref`` is whatever identifier
    the source POS uses for the bill (document number + fiscal year).

    ``irn`` + ``ack_no`` + ``ack_dt`` are populated once IRP registers the
    invoice (or tells us it was already registered via 2150).
    """
...
class IrpAttempt(models.Model):
    """
    One row per HTTP attempt. The raw source of truth for latency + error
    stats. Append-only; never mutated after insert.
    """
...
class DeadLetter(models.Model):
    """
    Terminal-state row. Created once by the retry state machine when a
    submission is classified as non-retryable (or retries exhausted). The
    operator resubmit tool reads from here and can flip a submission back
    to ``pending`` after the underlying cause is fixed.
    """
