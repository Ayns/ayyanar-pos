...
case we raise a PAYLOAD_DIVERGENCE anomaly (surfaced, not silently overwritten).
"""
...
class DrainSink:
    ...
    In production, this would be replaced by the HTTP client that ships
    batches to the cloud API endpoint.
    """
    ...
    def ingest(self, batch: Iterable[IngestBatchItem]) -> list[tuple[str, int]]:
        if not self.online:
            raise ConnectionError("cloud sink offline")
        items = list(batch)
        if not items:
            return []
